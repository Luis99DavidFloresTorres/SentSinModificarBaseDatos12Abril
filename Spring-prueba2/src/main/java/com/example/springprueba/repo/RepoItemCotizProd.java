...
public interface RepoItemCotizProd extends JpaRepository<ItemCotizProducto, Long> {
    @Query(value = "SELECT new com.example.springprueba.model.ItemCotizProducto(co,pr, (ic.precio*ic.cantidad), ic.precio, ic.cantidad) from itemcotizprod ic inner join cotizproducto co on ic.cotizProducto.id = co.id inner join producto pr on ic.producto.id = pr.id where co.proveedor= :proveedor order by co.nrodoc asc")
